...
def start_training(
    ...
    # 0. Override config with version if version_id is provided
    from app.models import DatasetVersion
    if body.version_id:
        version = db.query(DatasetVersion).filter(
            DatasetVersion.id == body.version_id,
            DatasetVersion.project_id == project_id
        ).first()
        if not version:
            raise HTTPException(status_code=404, detail="Dataset version not found")
        
        # Override body with version config
        body.imgsz = version.image_size
        body.splits = SplitConfig(train=version.train_split, val=version.val_split, test=version.test_split)
        body.preprocessing = PreprocessingConfig(**version.preprocessing)
        body.augmentations = AugmentationConfig(**version.augmentations)
    ...
    any_augmentation_enabled = any([
        getattr(aug, 'flip', False), getattr(aug, 'rotate90', False), getattr(aug, 'crop', False),
        getattr(aug, 'rotation', False), getattr(aug, 'shear', False), getattr(aug, 'brightness', False),
        getattr(aug, 'exposure', False), getattr(aug, 'blur', False), getattr(aug, 'noise', False),
        getattr(aug, 'motionBlur', False), getattr(aug, 'cameraGain', False)
    ]) if aug else False
...
class AugmentationPreviewRequest(BaseModel):
    augmentations: dict  # Flexible dict to allow partial augmentation configs
    image_size: int = 640
    single_augmentation: bool = False  # Flag indicating if this is a single augmentation preview


def apply_augmentations(image: np.ndarray, config) -> np.ndarray:
    """Apply augmentations to an image and return the augmented image.
    
    Args:
        image: numpy array of the image
        config: AugmentationConfig object or dict with augmentation flags
    """
    import cv2  # Local import to avoid startup issues
    augmented = image.copy()
    
    # Handle both dict and AugmentationConfig objects
    def get_attr(key, default=False):
...
def get_augmentation_preview(
    ...
    try:
        ...
        for img in sample_images:
            try:
                # Load the actual image from S3
                image_bytes = get_file_from_s3(img.s3_key)
                if not image_bytes:
                    continue
                ...
                # Resize to requested preview size
                image = cv2.resize(image, (body.image_size, body.image_size))
                ...
                # Apply augmentations
                augmented = apply_augmentations(image, body.augmentations)
        ...
        return {
            ...
            "augmentations_applied": body.augmentations if isinstance(body.augmentations, dict) else body.augmentations.dict()
        }
...
@router.delete("/{project_id}/train/history/{job_id}")
def delete_training_history(
    ...
    """Delete a training job and its associated models."""
    job = db.query(TrainingJob).filter(TrainingJob.id == job_id, TrainingJob.project_id == project_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="Training job not found")

    # Keep base models protected
    models = db.query(ModelVersion).filter(ModelVersion.training_job_id == job_id).all()
    ...
    return {"status": "success", "message": "Training history deleted successfully"}